...
public class ImageGridAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (itemViewType == ITEM_TYPE_CAMERA) {
            int listItemId = MResource.getIdByName(mActivity,"layout","txwp_adapter_camera_item");
            convertView = LayoutInflater.from(mActivity).inflate(listItemId, parent, false);
            ...
            if (convertView == null) {
                int listItemId = MResource.getIdByName(mActivity,"layout","txwp_adapter_image_list_item");
                convertView = LayoutInflater.from(mActivity).inflate(listItemId, parent, false);
                convertView.setLayoutParams(new AbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, mImageSize)); //让图片是个正方形
            ...
            holder.cbCheck.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (holder.cbCheck.isChecked() && mSelectedImages.size() >= selectLimit) {
                        int strId = MResource.getIdByName(mActivity,"string","txwp_select_limit");
                        Toast.makeText(mActivity.getApplicationContext(), mActivity.getString(strId, selectLimit), Toast.LENGTH_SHORT).show();
    ...
    private class ViewHolder {
        ...
        public ViewHolder(View view) {
            rootView = view;
            int ivThumbId = MResource.getIdByName(mActivity,"id","iv_thumb");
            int maskId = MResource.getIdByName(mActivity,"id","mask");
            int cbCheckId = MResource.getIdByName(mActivity,"id","cb_check");

            ivThumb = (ImageView) view.findViewById(ivThumbId);
            mask = view.findViewById(maskId);
            cbCheck = (SuperCheckBox) view.findViewById(cbCheckId);
        }
    }
