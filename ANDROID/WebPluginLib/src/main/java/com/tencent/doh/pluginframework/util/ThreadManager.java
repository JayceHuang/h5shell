...
import com.tencent.doh.BuildConfig;
...
public final class ThreadManager
{
	...
	/**
	 * AsyncTask的默认线程池Executor. 负责长时间的任务(网络访问)
	 * 默认3个线程 
	 */
	...
	private static Executor initNetworkExecutor()
	{
		...
		if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB)
		{
			if(DEBUG_THREAD)
			{
				try
				{
					ThreadPoolExecutor executor = new ThreadPoolExecutor(3, 128, 1, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(10),
							new ThreadFactory()
							{
...
//			        QLog.e("ThreadManager", QLog.CLR, e.getMessage(), e);
//			    }
	...
	public static void executeOnNetWorkThread(Runnable run)
    {
        try {
//        	if(QLog.isColorLevel()) {
//        		NETWORK_EXECUTOR.execute(new WrapRunnable(run, "NetWorkThread"));
//        	} else {
...
//        	}
	...
	{
//		if(QLog.isColorLevel()) {
//			getSubThreadHandler().post(new WrapRunnable(run, "SubThread"));
//		} else {
...
//		if(QLog.isColorLevel()) {
//			getFileThreadHandler().post(new WrapRunnable(run, "FileThread"));
//		} else {
			...
			if(bq!=null&&!bq.isEmpty()){
				try {
					for(Runnable bu:bq){
						...
						field.setAccessible(true);
//						TraceUtils.traceBegin("Queue details." + field.get(bu).getClass());
						}catch (NoSuchFieldException e1) {
//							TraceUtils.traceBegin("Queue details." + bu.getClass());
					}
					}
