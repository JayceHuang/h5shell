...
public class FontSliderBar extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        createBar();
        createThumbs();
        moveThumbToIndex(mCurrentIndex);
    }
    ...
    public FontSliderBar setTickCount(int tickCount) {
        if (isValidTickCount(tickCount)) {
            ...
            LogUtils.e(TAG, "tickCount less than 2; invalid tickCount.");
    ...
    private void moveThumbToIndex(int index){
        if (mBar == null)
            return;
        float centerCoordinate = mBar.getCoordinateByIndex(index);
        mThumb.setX(centerCoordinate);
    }
